...
public:
    ...
    static void SimulateSuccessfulWipe(ProgressCallback callback, int passes = 1,
                                       std::chrono::milliseconds delay = std::chrono::milliseconds{
                                           10}) {
        for (int pass = 1; pass <= passes; ++pass) {
            for (int pct = 0; pct <= 100; pct += 25) {
                if (callback) {
                    WipeProgress progress{.bytes_written = static_cast<uint64_t>(pct * 10'000),
        ...
        // Send completion
        if (callback) {
            WipeProgress complete{.bytes_written = 1'000'000,
                                  .total_bytes = 1'000'000,
                                  .current_pass = passes,
                                  .total_passes = passes,
                                  .percentage = 100.0,
    ...
    static void SimulateFailedWipe(ProgressCallback callback, const std::string& error_message) {
        if (callback) {
            WipeProgress progress{.bytes_written = 250'000,
                                  .total_bytes = 1'000'000,
                                  .current_pass = 1,
                                  .total_passes = 1,
                                  .percentage = 25.0,
