...
public:
    ...
    MOCK_METHOD((std::expected<void, util::Error>), unmount_disk,
                (const std::string& path), (override));
    MOCK_METHOD(bool, is_disk_writable, (const std::string& path), (override));
    MOCK_METHOD((std::expected<uint64_t, util::Error>), get_disk_size,
                (const std::string& path), (override));
    MOCK_METHOD((std::expected<void, util::Error>), validate_device_path,
                (const std::string& path), (override));
    ...
    static std::shared_ptr<MockDiskService> CreateNiceMock() {
        ...
        ON_CALL(*mock, is_disk_writable(testing::_))
    ...
    static DiskInfo CreateTestDisk(const std::string& path = "/dev/sda",
                                   uint64_t size = 1024ULL * 1024 * 1024,
                                   bool mounted = false) {
        return DiskInfo{
            .path = path,
            .model = "Test Disk",
