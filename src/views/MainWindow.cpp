...
namespace {
    ...
    auto on_countdown_tick(gpointer user_data) -> gboolean {
        ...
        if (data->seconds_remaining > 0) {
            // Update button text with countdown - pad to 2 digits to prevent layout shift
            auto button_text = std::format("Confirm ({:2d})", data->seconds_remaining);
...
void MainWindow::show_message(const MessageInfo& message) {
    ...
    // Set preferred width to prevent layout shift during countdown
    ...
    if (message.type == MessageInfo::Type::CONFIRMATION) {
        ...
        // Add confirm button with initial countdown text (pad to 2 digits)
        auto initial_text = std::format("Confirm ({:2d})", CONFIRMATION_COUNTDOWN_SECONDS);
...
void MainWindow::on_about_clicked(GtkWidget*, gpointer user_data) {
    ...
    adw_dialog_present(ADW_DIALOG(about), GTK_WIDGET(self->window_));
}
