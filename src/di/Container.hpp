 ...
 *
 * Provides a singleton pattern for accessing a global DI container.
 * Use sparingly - prefer constructor injection where possible.
 *
 * @example
 * ```cpp
 * // Configure at startup
 * ServiceLocator::configure([](Container& c) {
 *     c.register_type<IDiskService, DiskService>();
 * });
 *
 * // Resolve anywhere
 * auto service = ServiceLocator::resolve<IDiskService>();
 * ```
 */
class ServiceLocator {
public:
