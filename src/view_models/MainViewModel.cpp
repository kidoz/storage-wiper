...
void MainViewModel::update_can_wipe() {
    ...
    if (path.empty() || is_wipe_in_progress.get()) {
        can_wipe.set(false);
        wipe_command->raise_can_execute_changed();
        return;
    }
    ...
    // Check if disk is mounted - if so, allow it (start_wipe will offer to unmount)
    // This is needed because is_disk_writable() fails on mounted devices
    if (auto disk_info = find_disk_info(path); disk_info && disk_info->is_mounted) {
        can_wipe.set(true);
        wipe_command->raise_can_execute_changed();
        return;
    }

    // For unmounted disks, verify we have write access
    bool can = disk_service_->is_disk_writable(path);
    can_wipe.set(can);
    wipe_command->raise_can_execute_changed();
}
...
void MainViewModel::start_wipe() {
    ...
    // Check mount status FIRST - mounted disks need to be unmounted before we can check writability
    if (auto disk_info = find_disk_info(path)) {
        if (disk_info->is_mounted) {
            ...
            mounted_message << ".\n\n";
            mounted_message << "Do you want to unmount all partitions and proceed with wiping?";
    ...
    // For unmounted disks, verify write access
    ...
    show_message(MessageInfo::Type::CONFIRMATION, "Confirm Disk Wipe", message.str(),
                 [this](bool confirmed) {
                     if (confirmed) {
                         confirm_wipe(false);  // false = no unmount needed
                     }
                 });
}

void MainViewModel::unmount_and_wipe(const std::string& path) {
    // Show final confirmation dialog - unmount will happen via helper with root privileges
    std::ostringstream message;
    message << "Are you sure you want to wipe '" << path << "'?\n\n";
    message << "The disk will be automatically unmounted before wiping.\n\n";
    message << "Algorithm: " << wipe_service_->get_algorithm_name(selected_algorithm.get()) << "\n";
    message << "Description: " << wipe_service_->get_algorithm_description(selected_algorithm.get()) << "\n\n";
    message << "WARNING: This will permanently destroy ALL data on the disk!\n";
    message << "This action cannot be undone!";
    ...
    show_message(MessageInfo::Type::CONFIRMATION, "Confirm Disk Wipe", message.str(),
                 [this, path](bool confirmed) {
                     if (confirmed) {
                         selected_disk_path.set(path);
                         confirm_wipe(true);  // true = unmount first via helper
                     }
                 });
}

void MainViewModel::confirm_wipe(bool unmount_first) {
