...
public:
    ...
    auto unmount_disk(const std::string& path) -> bool override;
    [[nodiscard]] auto is_disk_writable(const std::string& path) -> bool override;
    [[nodiscard]] auto get_disk_size(const std::string& path) -> uint64_t override;
    [[nodiscard]] auto validate_device_path(const std::string& path) -> bool override;
...
private:
    ...
    void check_lvm_usage(const std::string& device_name, DiskInfo& info);
};